...
wgm_filt<-wgm|>
  select(c(WP5,wgt,PROJWT,FIELD_DATE,YEAR_CALENDAR,Q1,Q25,AgeCategories,Regions_Report))|>
  filter(!is.na(Q25))
...
#Bringing in chloropleth of countries
...
wgm_sci_age$Region<-factor(wgm_sci_age$Region,levels = c("World","Eastern Africa","Central Africa","North Africa","Southern Africa","Western Africa","Central America and Mexico","Northern America","South America","Central Asia","East Asia","Southeast Asia","South Asia","Middle East","Eastern Europe","Northern Europe","Southern Europe","Western Europe","Aus/NZ"))
...
wgm_sci_age$AgeRange<-factor(wgm_sci_age$AgeRange,levels = c("50+","30 to 49","15 to 29"))
...
ggplot(wgm_sci_age,aes(Prop*100,Region,fill=AgeRange),color="black")+
  ...
  theme_minimal()+
  theme(legend.position = c(0.85,0.8),panel.grid.minor.x = element_blank(),panel.grid.major.y = element_blank(),text = element_text(size=12),panel.background = element_rect(fill = "beige",color = "beige"),plot.background = element_rect(fill="beige",color="beige"),panel.grid.major.x = element_line(linetype = "dashed",colour = "grey"),plot.title.position = "plot")+
